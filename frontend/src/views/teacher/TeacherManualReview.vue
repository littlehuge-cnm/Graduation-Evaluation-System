...
<template>
  <div v-loading="loading">
    ...
    <!-- 预览弹窗 -->
    <el-dialog v-model="previewVisible" title="评价手册预览" width="90%" top="3vh" destroy-on-close
      ...
      <div v-loading="previewLoading" class="preview-doc">
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><td class="col-label">毕业设计（论文）题目</td><td colspan="2">{{ pv('g') }}</td></tr>
            <tr><td class="col-label">课题类别</td><td>A.工程设计B.科学研究C.技术开发D.其他</td><td class="col-select">{{ pv('h') }}</td></tr>
            <tr><td class="col-label">课题类型</td><td>A.真题B.模拟题（假题）C.真题假作</td><td class="col-select">{{ pv('i') }}</td></tr>
            <tr><td class="col-label">新旧课题</td><td>A.新题B.旧题</td><td class="col-select">{{ pv('j') }}</td></tr>
            <tr><td colspan="3" class="content-cell">课题研究的主要内容<br/>{{ pv('k') }}</td></tr>
            <tr><td colspan="3" class="content-cell">基本要求<br/>{{ pv('l') }}</td></tr>
            <tr><td colspan="3">导师签字：{{ pv('m') }}&emsp;{{ pv('n') }}年&emsp;{{ pv('o') }}月&emsp;{{ pv('p') }}日</td></tr>
            <tr><td colspan="3">系意见：{{ pv('q') }}系主任签字：{{ pv('r') }}&emsp;{{ pv('s') }}年&emsp;{{ pv('t') }}月&emsp;{{ pv('u') }}日</td></tr>
            <tr><td colspan="3">学院意见：{{ pv('v') }} 院长签字:{{ pv('w') }}&emsp;{{ pv('x') }}年&emsp;{{ pv('y') }}月&emsp;{{ pv('z') }}日</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><td class="content-cell">{{ pv('A') }}</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><th>检查项目</th><th>分值</th></tr>
            <tr><td>1．调研资料的获取能力（资料的检索、收集、整理、归纳能力；调研的范围、地点、领域内国内外的发展状况、已取得的应用成果、目前存在的问题等）</td><td>4</td></tr>
            <tr><td>2．课题方案设计的合理性（拟采用的设计方案的合理性、有效性与可行性）</td><td>4</td></tr>
            <tr><td>3．开题报告的规范性与质量</td><td>4</td></tr>
            <tr><td>开题报告总成绩（由小组给分）</td><td>{{ pv('H') }}</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><th>检查项目</th><th>分值</th><th>成绩</th></tr>
            <tr><td>对外文资料的阅读理解能力（深入理解外文资料的全文，译文能够做到兼顾上下文）</td><td>1</td><td>{{ pv('I') }}</td></tr>
            <tr><td>专业词语翻译的准确性（译文中专业术语准确、规范）</td><td>1</td><td>{{ pv('J') }}</td></tr>
            <tr><td>译文规范性与质量（译文的格式符合校论文规范、译文行文流畅）</td><td>1</td><td>{{ pv('K') }}</td></tr>
            <tr><td>外文翻译总成绩（指导教师给分）</td><td colspan="2">{{ pv('L') }}</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><th>检查项目</th><th>分值</th></tr>
            <tr><td>1．完成毕业设计进度情况（超前、正常、滞后）</td><td>5</td></tr>
            <tr><td>2．综合能力（独立工作能力、调研能力、对知识的综合运用能力等）</td><td>5</td></tr>
            <tr><td>3．已完成的部分毕业设计质量</td><td>5</td></tr>
            <tr><td>总成绩</td><td>{{ pv('M') }}</td></tr>
            <tr><td colspan="2" class="content-cell">检查小组评语<br/>{{ pv('N') }}<br/>检查组长签字：{{ pv('O') }}&emsp;{{ pv('P') }}年&emsp;{{ pv('Q') }}月&emsp;{{ pv('R') }}日</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><th>考核项目</th><th>分值</th><th>成绩</th></tr>
            <tr><td>1．设计（实验）方案、研究方案及软硬件方案设计能力</td><td>3</td><td>{{ pv('S') }}</td></tr>
            <tr><td>2．基本概念、基本理论的应用能力</td><td>3</td><td>{{ pv('T') }}</td></tr>
            <tr><td>3．分析问题、解决问题及知识综合运用能力</td><td>3</td><td>{{ pv('U') }}</td></tr>
            <tr><td>4．科学素养、学习态度、纪律表现</td><td>3</td><td>{{ pv('V') }}</td></tr>
            <tr><td>5．工作量及毕业设计（论文）规范与质量</td><td>3</td><td>{{ pv('W') }}</td></tr>
            <tr><td>合计</td><td>15</td><td>{{ pv('X') }}</td></tr>
            <tr><td colspan="3" class="content-cell">指导老师评语<br/>{{ pv('Y') }}<br/>指导老师签字:{{ pv('Z') }}&emsp;{{ pv('aa') }}年&emsp;{{ pv('ab') }}月&emsp;{{ pv('ac') }}日</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><th>考核项目</th><th>分值</th><th>成绩</th></tr>
            <tr><td>1．毕业设计（论文）规范性与质量（格式的规范性，是否符合要求）</td><td>4</td><td>{{ pv('ad') }}</td></tr>
            <tr><td>2．基本理论和基本知识运用情况（运用与课题相关的基础理论和专业知识分析问题和解决问题的能力；理论、公式的正确性；概念是否清楚，应用是否合理）</td><td>4</td><td>{{ pv('ae') }}</td></tr>
            <tr><td>3．研究方案及设计方案（方案的正确性、逻辑性；论证的严密性，计算是否准确，有无表达错误）</td><td>4</td><td>{{ pv('af') }}</td></tr>
            <tr><td>4．毕业设计（论文）创新性（研究成果是否具有一定水平，有自己的见解，研究成果有一定的学术或应用价值）</td><td>3</td><td>{{ pv('ag') }}</td></tr>
            <tr><td>合计</td><td>15</td><td>{{ pv('ah') }}</td></tr>
            <tr><td colspan="3" class="content-cell">评阅老师评语（对学生毕业设计中完成的任务、涉及的知识、成果的数量与质量、体现的工作能力与创新精神、存在的问题等做出综合评价）：<br/>{{ pv('ai') }}<br/>评阅老师签字：{{ pv('aj') }}&emsp;{{ pv('ak') }}年&emsp;{{ pv('al') }}月&emsp;{{ pv('am') }}日</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><td>答辩日期</td><td>{{ pv('an') }}</td><td>学生姓名</td><td>{{ pv('ao') }}</td><td>记录人</td><td>{{ pv('ap') }}</td></tr>
            <tr><td colspan="6" class="content-cell">{{ pv('aq') }}</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><th>成绩评定项</th><th>评定项分值</th></tr>
            <tr><td>1．毕业设计（论文）陈述情况（是否简明扼要、思路清晰；语言表达准确，概念清楚，论点正确，分析归纳合理）</td><td>10分</td></tr>
            <tr><td>2．毕业设计（论文）水平（是否有独到的见解，富有新意，或对某些问题有较深刻的分析，有较高的学术水平或较高的实用价值）</td><td>10分</td></tr>
            <tr><td>3．毕业设计（论文）工作量评价（工作量饱满程度，独立完成情况）</td><td>10分</td></tr>
            <tr><td>4．答辩情况（是否能够准确深入地回答所提出的问题，基本概念清楚，有理有据，语言表达能力、是否得体）</td><td>10分</td></tr>
            <tr><td>答辩成绩</td><td>{{ pv('ar') }}</td></tr>
            <tr><td colspan="2" class="content-cell">答辩小组评语<br/>{{ pv('as') }}<br/>答辩小组组长签字：{{ pv('at') }}<br/>答辩小组成员签字：{{ pv('au') }}<br/>{{ pv('av') }}年&emsp;{{ pv('aw') }}月&emsp;{{ pv('ax') }}日</td></tr>
          </table>
        </div>
        ...
        <div class="preview-page">
          ...
          <table class="preview-table">
            <tr><th>成绩评定项</th><th>得分</th><th>备注</th></tr>
            <tr><td>开题报告12%</td><td>{{ pv('ay') }}</td><td>{{ pv('az') }}</td></tr>
            <tr><td>英文翻译3%</td><td>{{ pv('ba') }}</td><td>{{ pv('bb') }}</td></tr>
            <tr><td>中期检查15%</td><td>{{ pv('bc') }}</td><td>{{ pv('bd') }}</td></tr>
            <tr><td>指导教师15%</td><td>{{ pv('be') }}</td><td>{{ pv('bf') }}</td></tr>
            <tr><td>评阅教师15%</td><td>{{ pv('bg') }}</td><td>{{ pv('bh') }}</td></tr>
            <tr><td>答辩40%</td><td>{{ pv('bi') }}</td><td>{{ pv('bj') }}</td></tr>
            <tr><td>总分100%</td><td colspan="2">{{ pv('bk') }}</td></tr>
            <tr><td>评定等级</td><td colspan="2">{{ pv('bl') }}</td></tr>
            <tr><td colspan="3" class="content-cell">答辩委员会评语<br/>{{ pv('bm') }}<br/>答辩委员会主任签字（盖章）{{ pv('bn') }}<br/>答辩委员会秘书签字（盖章）{{ pv('bo') }}</td></tr>
          </table>
